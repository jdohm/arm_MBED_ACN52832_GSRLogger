...
// TOD - done?
//SparkFun_Bio_Sensor_Hub::SparkFun_Bio_Sensor_Hub(PinName resetPin, PinName mfioPin, uint8_t address) { 
SparkFun_Bio_Sensor_Hub::SparkFun_Bio_Sensor_Hub(DigitalOut& resetPin, DigitalInOut& mfioPin, uint8_t address) { 
  
  //_resetPin = resetPin; 
  //_mfioPin = mfioPin;
  _address = address; 
  //DigitalInOut _intmfioPin(mfioPin);
  _mfioPin = &mfioPin;
  //_mfioPin(mfioPin);
  _mfioPin->output();
  //DigitalInOut _resetPin(resetPin); // Set these pins as output
  //DigitalOut _intresetPin(resetPin);
  _resetPin = &resetPin;
}
...
// TOD - done?
uint8_t SparkFun_Bio_Sensor_Hub::begin( I2C &wirePort ) {
...
// TOD - DONE?
uint8_t* SparkFun_Bio_Sensor_Hub::readFillArray(char _familyByte, uint8_t _indexByte,\
