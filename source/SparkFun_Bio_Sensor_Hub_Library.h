/* sparkfun Bio Sensor library modified to work in arm MBED 
 * modified by j.dohm
 * date: 2020-07-15
 */
...
#define ENABLE_CMD_DELAY          45ms // Milliseconds
#define CMD_DELAY                 60ms  // Milliseconds
...
class SparkFun_Bio_Sensor_Hub
{
  public:  
    ...
    // Constructor ----------
    SparkFun_Bio_Sensor_Hub(DigitalOut&, DigitalInOut&, uint8_t address = 0xAA); 
    ...
    uint8_t begin( I2C &wirePort );
    //uint8_t begin( I2C &wirePort = Wire);
    ...
    uint8_t beginBootloader( I2C &wirePort ); 
    //uint8_t beginBootloader( I2C &wirePort = Wire); 
  ...
  private:   
    ...
    // the given sensor. 
    uint8_t enableWrite(char,  uint8_t, uint8_t);
    ...
    uint8_t writeByte(char, uint8_t, uint8_t, uint8_t);
    ...
    // indicating what you want to do, a delay, and then a read to confirm positive
    // transmission. 
    uint8_t writeByte(char, uint8_t, uint8_t, uint16_t);
    ...
    uint8_t readByte(char, uint8_t, uint8_t); 
    ...
    uint8_t* readFillArray(char, uint8_t, uint8_t, uint8_t array[]);
};
#endif
